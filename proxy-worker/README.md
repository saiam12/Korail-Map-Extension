...
배포 결과가 `https://korail-map-proxy.<account>.workers.dev`라면 확장 프로그램의 `src/background/background-config.js`를 다음처럼 설정합니다.
...
- Worker 로그에 Client ID, Client Secret, Kakao REST API 키 또는 CARTO Basemaps 키를 출력하지 않습니다.
